...
GLOBAL_EN = (
    ...
    "PLACEMENT IS INTENTIONAL: where the layout puts an object is dictated by staging — "
    "by characters and camera that may not be drawn — so positions that look arbitrary are "
    "deliberate. Keep every object exactly where the layout places it; never re-space, "
    "align, center or symmetrize furniture and props.\n"
...
GLOBAL_JP = (
    ...
    "配置は意図的: 原図内の物の位置は演出（描かれていないキャラクターやカメラ）で決まっており、"
    "不自然に見える配置も意図されたもの。全ての物を原図の位置のまま描く — "
    "等間隔化・整列・センタリング・対称化・再配置をしない。\n"
