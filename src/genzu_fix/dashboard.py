...
<style>
 ...
 .cut{font-weight:700;white-space:nowrap} .prompt{max-width:320px;color:#444}
 ...
 .thumbs{display:flex;gap:6px;flex-wrap:wrap}
    ...
    th = ("<tr><th>カット</th><th>原図</th><th>美術ボード</th><th>プロンプト</th>"
          "<th>生成結果</th><th>状態</th><th>更新</th></tr>")
    ...
    for row in rows:
        ...
        prompt = html.escape(_short(latest.get("prompt", "")))
        ...
        body.append(
            ...
            f'<td>{boards_html}</td>'
            f'<td class="prompt">{prompt or "—"}</td>'
            f'<td>{thumbs_html}</td>'
