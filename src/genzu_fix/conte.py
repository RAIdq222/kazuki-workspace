...
def _extraction_prompt_page(glossary: str) -> str:
    return (
        ...
        "2枚目=『ページ全体』= このページの表全体。**カット番号と各コマの区切り(どの行が新カットでどの行が続きか)は、"
        "この“本物の配置”の画像で読む**。各行の左端 scene欄に『丸囲み数字 / 縦棒｜ / 空白』がある。\n"
        "3枚目=『scene番号欄・拡大』= scene欄(左端)だけを縦に拡大した1枚。丸数字と縦棒の“形”の確認用。\n"
        "4枚目以降=各コマを上から順に**列ごとに4枚**渡す(本文の正確な文字はこちらで読む): \n"
        "  「picture(絵)」/「action欄」/「dialogue欄(セリフのみ)」/「time欄」。\n"
        "**各列画像はその列の中身だけ。ラベルの列以外の内容を絶対に混ぜない**"
        "(例: 『action欄』画像の文字を dialogue に入れない。『dialogue欄』にはセリフ以外を入れない)。\n"
        "**番号・区切り＝2枚目(ページ全体)で判断。本文の文字＝4枚目以降の各列画像から読む**。"
        "コマの順序(上から)は3者で一致している。\n\n"
        "【ページ種別の判定（最初に行う）】\n"
        ...
        "・**『ページ全体』画像を上から下へ行ごとに見て、各行の左端scene欄を次の3通りに必ず分類する**。"
        "**丸で囲まれているか否か**が決め手(丸囲み=数字、丸の無い裸の縦線/十字=継続):\n"
...
def extract2(page_paths: list[str], out_json: str = "runs/conte_frames_v2_ep7.json",
    ...
    for pp in page_paths:
        ...
        # 2枚目=ページ全体（番号と各コマの位置関係を“本物の配置”で読ませる＝行ズレ防止の核心）
        fb, fc = _crop_b64(img, (0, 0, img.width, img.height), min_w=1100, max_edge=1980)
        # 3枚目=scene番号欄をページ全体で1枚に拡大（丸数字と縦棒の“形”をはっきり見せる）
        ...
        if debug_crops:
            ...
            fc.save(os.path.join(d0, "page_full.png"))
            scc.save(os.path.join(d0, "scene_column_full.png"))
        else:
            crops.append(("ページ上部ヘッダ(No.と表頭)", hb))
            crops.append((f"ページ全体(上から{len(bands)}コマ。番号と各コマの位置はこの本物の配置で読む)", fb))
            crops.append(("scene番号欄・拡大(丸数字=新カット/縦棒・空白=続き。形の確認用)", scb))
