...
# 楼閣/客間区/廊下) + 塀・広場・点景。layout_kogu.py に整合
# 実行: python src/stage3d/palace/scenes_kogu_all.py -- --views W,P1,C2,Y \
#          --samples 64 --res 1600x900 --out work/renders --tag all1 \
#          --blend work/palace_kogu_all.blend
import math
import os
import sys
...
import layout_kogu as LK  # noqa: E402
...
# カメラは configs/kogu.json を単一の情報源とする (Issue #6 P0-1: 手転記の廃止)
# config は three.js 座標系 (x, z, -y) なので blender 座標へ逆変換する
import json  # noqa: E402

with open(os.path.join(os.path.dirname(_HERE), "configs", "kogu.json"),
