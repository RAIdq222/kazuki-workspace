...
M = dict(
    ...
    body_red=mat("body_red", (0.45, 0.16, 0.11), rough=0.85),  # 皇宮の赤壁
    roof_grey=mat("roof_grey", (0.29, 0.32, 0.34), rough=0.8),   # 宮邸系 灰瓦
    ...
    fig=mat("fig", (0.15, 0.30, 0.75), rough=0.6),          # 人型スケール(1.65m)
)
...
def bmat(b):
    return M["body_red"] if b.get("body") == "red" else M["body"]
...
def grand_stair(pid, x, y_front, h, width=16):
    """高基壇用の大階段: 2連(踊り場付き)+甬道(参道)を略記."""
    run = h * 2.6
    half = run / 2
    ang = -math.atan2(h / 2, half)
    box(f"{pid}_stair1", width, half + 2, 1.0,
        (x, y_front - run + half / 2 - 1, h * 0.2), M["stone"], rot=(ang, 0, 0))
    box(f"{pid}_landing", width + 4, 4.5, 1.0, (x, y_front - half, h * 0.5), M["stone"])
    box(f"{pid}_stair2", width, half + 2, 1.0,
        (x, y_front - half / 2 + 1.0, h * 0.72), M["stone"], rot=(ang, 0, 0))
    for sx in (-1, 1):  # 階段両脇の欄干 (白石高欄の略記)
        box(f"{pid}_srail{'we'[sx > 0]}", 0.7, run + 2, 0.9,
            (x + sx * (width / 2 + 0.6), y_front - half, h * 0.5 + 0.5),
            M["wallw"], rot=(ang, 0, 0))
    # 甬道: 広場から階段へ続く一段高い参道
    box(f"{pid}_app", 10, 14, 0.8, (x, y_front - run - 7, 0.4), M["stone"])
...
def build(b):
    ...
    if k == "plaza":
        ...
        for i, (rx, ry, rr) in enumerate([(38, -16, 2.0), (-30, 30, 2.4), (20, 42, 1.6)]):
            sphere(f"{b['id']}_rock{i}", rr, (x + rx, y + ry, rr * 0.5), M["stone"],
                   scale=(1.3, 1.0, 0.8))
    elif k == "pond":
        plane(b["id"], b["w"], b["d"], (x, y, 0.06), M["water"])
        box(b["id"] + "_bridge", 3, b["d"] * 0.7, 0.5, (x + b["w"] * 0.2, y, 0.9),
            M["stone"])
    elif k == "censer":
        cyl(b["id"], 1.3, 2.6, (x, y, 1.3), M["bronze"])
        for i, (dx, dy, sw, sd) in enumerate([(0, -4, 9, 0.4), (0, 4, 9, 0.4),
                                              (-4.5, 0, 0.4, 8), (4.5, 0, 0.4, 8)]):
            box(f"{b['id']}_rail{i}", sw, sd, 0.9, (x + dx, y + dy, 0.45), M["wallw"])
    elif k == "wall":
        ...
        for t in range(3):
            box(f"{b['id']}_t{t}", w, d, 3.2, (x, y, z + 1.6), bmat(b), rot=(0, 0, rz))
        ...
        box(f"{b['id']}_terr", b["w"] + 8, b["d"] + 8, th, (x, y, th / 2),
            M["stone"], rot=(0, 0, rz))
        box(f"{b['id']}_body", b["w"], b["d"], 8, (x, y, th + 4), bmat(b), rot=(0, 0, rz))
        ...
        box(f"{b['id']}_body", b["w"], b["d"], 5.5, (x, y, 2.75), bmat(b), rot=(0, 0, rz))
        hip_roof(f"{b['id']}_roof", b["w"], b["d"], 2.8, (x, y, 5.5), roof_mat(st), rot_z=rz)
    elif k == "hall2":
        t = b["terrace"]
        box(f"{b['id']}_terr1", t["w"], t["d"], t["h"] * 0.55, (x, y, t["h"] * 0.275),
            M["stone"], rot=(0, 0, rz))
        box(f"{b['id']}_terr2", t["w"] - 6, t["d"] - 6, t["h"] * 0.45,
            (x, y, t["h"] * 0.775), M["stone"], rot=(0, 0, rz))
        z0 = t["h"]
        box(f"{b['id']}_body", b["w"], b["d"], 11, (x, y, z0 + 5.5), bmat(b), rot=(0, 0, rz))
        ...
        grand_stair(b["id"], x, y - t["d"] / 2, t["h"])
    elif k == "wing":
        bld_hall(b, eave=6.5, ridge=4.5,
                 terr=dict(w=b["w"] + 4, d=b["d"] + 4, h=1.4), style=st)
    elif k == "hall":
        bld_hall(b, eave=11, ridge=7.5,
                 terr=dict(w=b["w"] + 6, d=b["d"] + 6, h=2.2), style=st)
    elif k == "hall_s":
        bld_hall(b, eave=5.5, ridge=3.5, style=st)
    elif k == "tower2":
        box(f"{b['id']}_b1", b["w"], b["d"], 5.5, (x, y, 2.75), bmat(b), rot=(0, 0, rz))
        ...
        box(f"{b['id']}_body", b["w"], b["d"], 3.2, (x, y, 1.6), bmat(b), rot=(0, 0, rz))
        ...
        hall = dict(id=b["id"] + "_hall", x=x, y=y + d / 4, w=w * 0.45, d=9,
                    face="S", style=b.get("style", "grey"), body=b.get("body"))
...
for wdef in L.WALLS:
    ...
    box(wdef["id"], length, 0.8, wdef["h"], (cx, cy, wdef["h"] / 2), M["wallw"], rot=(0, 0, rz))

for i, (tx, ty, tr) in enumerate(L.TREES):
    cyl(f"trunk{i}", tr * 0.12, tr, (tx, ty, tr * 0.5), M["bronze"], verts=8)
    sphere(f"tree{i}", tr, (tx, ty, tr * 1.35), M["tree"], scale=(1, 1, 0.85))

for i, (fx, fy) in enumerate(getattr(L, "FIGURES", [])):
    cyl(f"fig{i}", 0.22, 1.3, (fx, fy, 0.65), M["fig"], verts=10)
    sphere(f"fig{i}_h", 0.16, (fx, fy, 1.48), M["fig"])

_gd = L.SITE["y1"] - L.SITE["y0"]
plane("ground", L.SITE["x_half"] * 2 + 40, _gd + 50,
      (0, (L.SITE["y0"] + L.SITE["y1"]) / 2, 0), M["ground"])

set_world((0.62, 0.70, 0.82), 0.55)
sun_light("sun", rot=(math.radians(50), 0, math.radians(140)), energy=3.0, angle_deg=3)

cams = {}
for name, c in L.CAMS.items():
    cams[name] = add_camera(f"cam_{name}", c["loc"], c["look"], lens=c["lens"])
    cams[name].data.clip_end = 700.0

render_cli(cams, argv=argv, default_res="1600x900", exposure=0.8)
