...
- **佐々木とピーちゃん第2期(SP2) #10 = 着手（テスト）**。素材は会社Drive
  ...
  **genzu_trust モード導入（2026-07-17 黒江さん指摘）**: 「原図が正か」の判定なしに
  ...
  ※**2026-07-23訂正（Issue #5 D3-2）**: 「SP2は3Dレイアウト出し」は誤認。**SP2原図は普通の手書き**であり、
  正しいかどうかは原図次第・**カットごとに都度判断**（話数一括のtrust宣言は根拠を失った。GLOBAL_TRUSTの
  「3Dレイアウト出しで既に正しい」文面も虚偽状態＝Issue #5設計の実装で解消予定）。
  ...
  **D3〜D7回答済（2026-07-23・詳細はIssue #5コメント）**: ①納品物=カットの背景として構成物・カメラ位置が
    正確で必要十分なディテールを持つ清書 ②**SP2原図=手書き（3Dレイアウト説は誤認）・trustはカット単位判断**
    ③画調・情報量の正解=ボード/参考カット、**AIのくっきりしすぎ線はNG** ④原則白黒線画
    ⑤赤書き=AI提案→人承認＋判読不能/非合理はアラート ⑥原図vsコンテ=原図正＋食い違いは赤字等でアラート
