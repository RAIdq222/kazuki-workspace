...
最終更新: 2026-07-18（SP2参照ライブラリのToDoを追記）
トランク=main。新セッションは main から。可変状況は `docs/sessions.md`。

## 生成バックエンドの抽象化（黒江さん方針 2026-07-18・実装は必要になってから）
- [ ] Higgsfieldは暫定（黒江さんの個人クレジット・裁量範囲で使いやすいだけ）。最終的には
      最適な画像生成APIを選定して差し替える。準備:
      - 差し替え面は batch.py の `_hf_upload`/`_hf_generate`（他はバックエンド非依存）
      - バックエンド別の実測出力寸テーブル（現 `GPT_OUTPUT_SIZES`）を tier/model 別に持つ
      - プロンプトの効き（意匠辞書・引き癖対策・幾何の言語注入）はモデル依存＝乗り換え時に
        **GKV検証プロトコル（同条件生成→重ね合わせ→trust採点）をベンチとして再実行**して比較

## SP2: 参照画像ライブラリ（黒江さん指示 2026-07-18・実装は後回し）
- [ ] `00_資料_#10`（サンプルBG・実写参考・窓外素材 計491ファイル）を参照ライブラリ化する。
      **検索UIは作らない**（天使ちゃんスタジオのそれは制作の副産物）。目的は
      **AIがカットに合う参照画像を選ぶこと**であり、方式は**タグ付けではなく意味検索**
      （キャプション→埋め込み→stagingテキストとのコサイン近傍。天使ちゃんスタジオの
      scene_search.py 方式=Gemini gemini-embedding-001@768 を流用。参照ブランチ:
      `claude/horizontal-to-vertical-shorts-fzt88c` の src/shorts/scene_search.py・
      docs/scene-library-design.md）。
      取り込み時のキャプション生成→埋め込み→カットのstagingで近傍検索→上位を[IMAGES]の
      意匠辞書として生成に同梱、まで。選定結果は詳細画面で人が差し替え可能に。
...
## C. 衛生（2026-07 一部完了）
- [x] work/conte_crops を git rm --cached（gitignore 矛盾解消、4.5MBをトラック解除）
- [x] runs/README.md（正典/派生/旧の索引）＋ conte_v2 の正典明記（baseline/reocr は snapshot で保持）
- [x] run_gather.bat: git識別子ハードコード撤去（未設定は中断）＋ handoff投入前のサイズ確認ゲート
- [x] sessions.md 訂正: handoff は PSD無しセッションの原図経路として意図的にgit管理（scene_understanding/fetch_asset）
- [x] **handoff/ep7 の 51MB → 現状維持で決定（2026-07, ユーザー）**。scene_understanding/fetch_asset が
      使用中＋履歴rewriteの手間に見合わない（10カット規模では小さすぎる）。肥大はgatherのサイズ確認で抑制。
      ※ ep8以降で全カット規模になったら LFS/rewrite を再検討。
- [ ] 旧CSV/例（ep7_cut_board_map / cut_index.example）は tiny かつ doc参照のみ→ archive は任意（保留）
- [ ] docs 更新: NEXT-SESSION.md を archive／runbook のコンソール節を現UIに
