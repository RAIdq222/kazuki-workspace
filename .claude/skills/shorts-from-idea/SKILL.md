...
> に従う**: ①nano_banana_proでスタートフレームを静止画確定（チェックリスト目視）
> → ②合格画を start_image にして Seedance で動かす。三面図＋テキストの直行生成は禁止。
> 参照＝ルック参照（本編フレーム）＋**舞台参照（シーンの場所のセット）**＋小物設定画。
> さらに生成前に**シーンライブラリで同一シーンを検索**し、**ヒットした場合のみ**
> frame_media_id を参照に追加する（無い場合は無理に使わない）。
...
- 無ければ生成: `generate_image`（soul_2 / nano_banana_pro 等。aspect_ratio="9:16" で
  作っておくと start_image に使いやすい）
...
- カット間の一貫性: 同じ image_references を全カットに渡す。前カットの最終フレームに
  繋げたい場合は前カットの結果を `video_references` に渡すか、静止画を切り出して
  start_image に使う。
