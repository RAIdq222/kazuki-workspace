...
- シーン追加・キャプション修正をしたら再埋め込みが必要:
  scratchpadの embed_backfill.py 方式（export→RETRIEVAL_DOCUMENTで埋め込み→
  `POST /api/agent/scenes/vectors`）。docテキストは
  `${caption}（場所:${place}／行動:${action}／表情:${expression}／構図:${shot}／時間帯:${time_of_day}）`
  と一字一句一致させること。Gemini無料枠は約100件/分（429は70s待ちで再試行）
