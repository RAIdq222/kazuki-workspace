...
2. **EDL XML**: `python -m src.shorts.export_xmeml_edl <master> <segments.json> -o <name>_EDL.xml
   ...
   — 元動画1本への in/out タイムスタンプでシーケンスを組んだ xmeml。
   pathurl は src/shorts/data/master_local_dir.txt の置き場設定から絶対パスで書かれ、
   Resolve/Premiereで開いた瞬間にリンクされる。
   音声ベッドはA1/A2交互にクロスフェード分重ねて配置済み（クロスフェード適用で完パケと同尺）
3. **音声分離ファイル（必須）**: `ffmpeg -i <master> -vn -c:a copy "<マスター名>_audio.m4a"`
   を一緒に納品し、マスターと同じフォルダに置いてもらう。
   **DaVinci Resolveは映像入りファイルの音声のみ参照クリップを解決できない**
   （2026-07-09実測。linkやstereo属性でも直らない）ため、EDLの音声ベッドは
   この音声専用ファイルを参照する（エクスポータの既定動作）。
   Premiere専用と分かっている場合のみ `--embedded-audio` で旧方式に戻せる
