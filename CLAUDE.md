...
## 1. これは何のプロジェクトか
商用アニメの**背景原図(genzu)の修正**を自動化する**複数作品対応のプラットフォーム**。
ラフな原図PSDを受け取り、GPT Image 2(Higgsfield)で**修正・清書した原図**に整えて、
同じPSDへ「AI原図修正」レイヤーとして戻す（**原図→原図**。入口も出口も原図であり、
別スタイルへの変換ではない。画調の正解は作品ごとの参考資料に従う＝`runs/works.json` の style_refs）。
...
| コンソールのチーム共有手順 | `docs/deploy-console.md`（間借り機＋Tailscale＋トークン認証） |
| TODO | `docs/TODO.md` |
...
- **パースを手で詰める/直す（エディタ）** → `python run_perspective_editor.py`（既定 :8770）。
  ...
  「自動推定(cv/vision/hybrid)」で初期値を流し込み微調整→保存はブラウザのネイティブ保存ウィンドウ:
  「PNG保存」=焼き込みPNGを**ブラウザ側でフル解像度生成**（サーバ非依存＝固まらない・空ファイルに
  ならない。太さ・密度を反映）し、`showSaveFilePicker` でフォルダ/ファイル名を選んで保存。
  「パース線+」=消失点から個別のパース線（レイ）を1本ずつ追加（クリックで選択・ドラッグで角度・Delで削除）。
  「参考画像+」=写真等を追加読込し、四隅ハンドルの射影変形（ホモグラフィ）でシーンのパースに
  合わせて投影配置（アタリ・ラフ用。透明度スライダあり。PNG保存に焼き込み、透過PNGには入れない）。
  「透過PNG（線だけ）」=背景画像を描かず、アイレベル・パースガイド・人物垂直線だけを透過背景で保存。
  「JSON保存」=座標JSON。空ファイル防止のため blob はピッカーを開く前に生成。非対応ブラウザ
  (Firefox等)は自動でダウンロードへフォールバック（保存先指定はChrome/Edge）。サーバ側Tkダイアログは廃止
  （`/api/render` は残置＝CLI/検証用）。**背景がキャラ想定のパースと食い違う時は手置きで正す**。
