...
- **パースを手で詰める/直す（エディタ）** → `python run_perspective_editor.py`（既定 :8770）。
  「ファイルを開く…」(ネイティブのファイル選択ダイアログ) か画像のドラッグ&ドロップで開く。
  ...
  誤差1°未満は自動で水平へ補正）。線の太さは調整スライダあり（生成参照用に太め既定）。
  「自動推定(cv/vision/hybrid)」で初期値を流し込み微調整→「保存」はネイティブの保存先ダイアログで
  場所/名前を選べる（既定 `work/_perspective/<stem>/`、選んだ名前で `*.json`＋`*.png`）。
