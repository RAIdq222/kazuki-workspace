...
- **パースを手で詰める/直す（エディタ）** → `python run_perspective_editor.py`（既定 :8770）。
  ...
  「PNG保存」=`showSaveFilePicker` でフォルダを辿りファイル名を決めて保存（焼き込みPNGは
  `/api/render` がフルレゾで生成、太さ・密度を反映）、「JSON保存」=座標JSON。`showSaveFilePicker`は
  クリック操作中に先に開く必要があるため先にダイアログ→後でPNG生成の順。非対応ブラウザ(Firefox等)は
  自動でダウンロードへフォールバック（保存先指定はChrome/Edge）。サーバ側Tkダイアログは廃止。
