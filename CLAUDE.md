...
- **パースを手で詰める/直す（エディタ）** → `python run_perspective_editor.py`（既定 :8770）。
  ...
  「自動推定(cv/vision/hybrid)」で初期値を流し込み微調整→「保存」はOSネイティブの保存先ダイアログ
  （フォルダ移動＋ファイル名＋保存ボタン、`/api/pick_save`→`/api/save`）で、選んだ場所に
  焼き込みPNG（太さ・密度を反映）＋座標JSONを書き出す。ダイアログを開けない環境のみブラウザの
  保存ダイアログ/ダウンロードへフォールバック。**背景がキャラ想定のパースと食い違う時は手置きで正す**。
