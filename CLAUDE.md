...
- **パースを手で詰める/直す（エディタ）** → `python run_perspective_editor.py`（既定 :8770）。
  ブラウザで画像パスを入れて開き、アイレベル・消失点をドラッグで配置すると消失点へ収束する
  パースガイドを自動描画（1点〜複数消失点＝ジブリ風にも対応、鉛直消失点/人物垂直線も可）。
  「自動推定(cv/vision/hybrid)」で初期値を流し込み微調整→「保存」で `work/_perspective/<stem>/`
  に `<stem>.edit.json`＋`<stem>.edit.png`。**背景がキャラ想定のパースと食い違う時は手置きで正す**。
